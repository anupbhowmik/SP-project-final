 ...
 * - shared_prompt_tokens: sequences can be grouped to share common prefixes
 * - prompt_tokens: base shared prefix + random extra tokens
 ...
 */
 ...
 SequenceWork* generate_workload(const SimConfig* cfg) {
    ...
    for (size_t i = 0; i < cfg->num_sequences; ++i) {
        ...
        // generate a random amout of output tokens uniformly between [min_gen_tokens, max_gen_tokens].
